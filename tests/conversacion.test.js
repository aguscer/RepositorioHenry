...
const SABADO_20 = new Date('2026-08-22T23:00:00Z'); // local abierto

function decir(telefono, texto, fecha = SABADO_20) {
...
test('toma el pedido completo y lo deja listo para la caja', () => {
  ...
  assert.equal(pedido.subtotal, 18200);
  assert.equal(pedido.envio, 1500);
  assert.equal(pedido.total, 19700);
...
test('después de confirmar, el bot deja de responder para que atienda una persona', () => {
  ...
  for (const mensaje of ['quiero pedir 1 muzzarella', 'listo', 'retiro', 'Ana', 'efectivo', 'dale']) {
...
test('avisa cuando el local está cerrado pero igual toma el pedido', () => {
  const lunes = new Date('2026-08-24T23:00:00Z');
